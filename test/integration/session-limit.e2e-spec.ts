...
describe('Session limit concurrency (e2e)', () => {
  ...
  let sessionIssueUseCase: ISessionIssueUseCase;
  ...
  beforeAll(async () => {
    ...
    sessionIssueUseCase = app.get<ISessionIssueUseCase>(SESSION_ISSUE_USE_CASE);
  ...
  it('should not block session issuance for a different user', async () => {
    ...
    try {
      ...
      const secondIssue = sessionIssueUseCase.execute(
        secondUser.id,
        '127.0.0.2',
        {
          browserName: 'Firefox',
  ...
  it('should revoke the least recently used session and keep recent activity valid', async () => {
    ...
    try {
      ...
      const phone = await sessionIssueUseCase.execute(
        user.id,
        '127.0.0.1',
        {
          browserName: 'Chrome',
      ...
      const laptop = await sessionIssueUseCase.execute(
        user.id,
        '127.0.0.2',
        {
          browserName: 'Firefox',
      ...
      const newest = await sessionIssueUseCase.execute(
        user.id,
        '127.0.0.3',
        {
          browserName: 'Safari',
  ...
  it('should use creation time as a deterministic tie-break for equal activity', async () => {
    ...
    try {
      configService.set('MAX_ACTIVE_SESSIONS', 3);
      const older = await sessionIssueUseCase.execute(
        user.id,
        '127.0.0.1',
        {
          browserName: 'Chrome',
      ...
      const newer = await sessionIssueUseCase.execute(
        user.id,
        '127.0.0.2',
        {
          browserName: 'Firefox',
      ...
      configService.set('MAX_ACTIVE_SESSIONS', 2);
      await sessionIssueUseCase.execute(
        user.id,
        '127.0.0.3',
        {
          browserName: 'Safari',
