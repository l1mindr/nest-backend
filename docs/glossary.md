...
| **CSRF Token** | Random hex string stored in a readable `csrf_token` cookie. Sent as `X-CSRF-Token` header on unsafe methods. |
...
| **Verification Code** | SHA-256 hashed code sent via email. 3-minute TTL. Used to activate accounts after registration. |
